...
Created by mmiyaji on 2013-01-05.
...
class Author(models.Model):
    ...
    # author_id = models.CharField(max_length = 100, default="", blank=True, null=True, unique=True, db_index=True)
    ROLL_CHOICES = (
        ('s','Student'),
        ('t','Teacher'),
        ('g','Graduated'),
    ...
    def get_entries(self, span=10, page=0, search_query=None, isvalid=True, order="-created_at", all=False, listvalue=None):
        ...
        自分の著作を返す
        """
        result,count = Entry.get_items(author=self, span=span, page=page, search_query=search_query, isvalid=isvalid, order=order, all=all, listvalue=listvalue)
        return result,count
    def get_entry_num(self):
        """
        自分の著作の数を返す
        """
        num = Entry.objects.filter(authors__exact=self).count()
        return num
    def get_groups(self):
        """
    ...
    @staticmethod
    def get_by_author_id(keyid=""):
        result=None
        try:
            result = Author.objects.get(author_id__exact=keyid.strip())
        except:
            result = None
        return result
    @staticmethod
...
def get_origin_upload_path(self, filename):
    return get_upload_path(self, filename, types="originals")
def get_thumb_upload_path(self, filename):
    return get_upload_path(self, filename, types="thumbs")
def get_upload_path(self, filename, types="originals"):
    root_path = settings.EXPORT_DIR+types+"/%Y/%m/%d/"
...
class Entry(models.Model):
    ...
    def get_items(author=None, span=10, page=0,
        ...
        # if True:
            # 検索対象のすべてのエントリー数とSPANで区切ったエントリーを返す
            result = Entry.objects.order_by(order).filter(isvalid=isvalid)
            if author:
    ...
    def get_by_pub_and_name(pub, name):
        ...
        try:
            ...
            result = Entry.objects.filter(published_at__exact=pub).filter(original_title__exact=name)[0]
        except:
            result = None
        return result

    @staticmethod
    def get_by_uuid(entry_uuid):
        result = None
        try:
            result = Entry.objects.filter(uuid__exact=entry_uuid)[0]
        except:
            result = None
        return result
        ...
        if isFirst:
            if not self.uuid:
                ...
                for i in range(0,10):
                    ...
                    if not Entry.get_by_uuid(cuuid):
                        break
                self.uuid = cuuid
