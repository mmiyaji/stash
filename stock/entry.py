...
def home(request):
    ...
    if request.GET.has_key('st'):
        ...
        try: # キャストで失敗したら降順表示
            ...
            pass
    entrys,entry_count = Entry.get_items(span=span, page=page, search_query=search_query,
    ...
    temp_values = {
        ...
        "entrys":entrys,
        "tags":Tag.get_all(),
        "auth_years":Entry.get_years(),
        "pub_years":Entry.get_years(),
        "page_list":page_list,
        "pages":pages,
    ...
    return render_to_response('entry/index.html',temp_values,
                              context_instance=RequestContext(request))

def detail(request, entry_uuid):
    """
    Case of GET REQUEST '/entry/<entry_uuid>/'
    個別の画像の詳細を表示するページ
    """
    rotate = None
    temp_values = Context()
...
@csrf_protect
def delete(request, entry_uuid):
    """
    Case of DELETE REQUEST '/entry/<entry_uuid>/delete/'
    対象画像の削除
...
@csrf_protect
def update(request, entry_uuid):
    """
    Case of UPDATE REQUEST '/entry/<entry_uuid>/update/'
    対象画像の更新
    ...
    if request_type == 'GET':
        ...
        if param['pubdate']:
            ...
            if p:
                # 年月日のみ更新
                entry.published_at = entry.published_at.replace(year=p.year, month=p.month, day=p.day)
        entry.save()
        # 元のページにリダイレクト ブラウザのキャッシュで更新されてない画面が出るのを防止
        return HttpResponseRedirect("/entry/%s/?update=%d" % (entry_uuid, datetime.datetime.now().microsecond))
    else:
        raise Http404
@csrf_protect
def rotate(request, entry_uuid):
    """
    Case of POST REQUEST '/entry/<entry_uuid>/rotate/'
    対象画像の回転
    POST リクエストにのみレスポンス
    """
    request_type = request.method
    ...
    if request_type == 'GET':
        ...
        entry.save()
        return HttpResponseRedirect("/entry/%s/?rotate=true" % entry_uuid)
